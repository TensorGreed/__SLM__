...
- **Hyperparameter grid bake-off** — a real sweep where **each grid cell is a normal `Experiment`**. Give it a set of LoRA ranks and learning rates (e.g. ranks `8, 16`, LRs `2e-4, 3e-4`); the platform materializes one experiment per cell under a shared `sweep_id`, trains each through the normal path (so all preflight / eval / gate machinery applies), and the `HyperparameterSweepPanel` plots **quality vs the chosen cost axis** as cells finish. Quality is the cell's eval pass-rate when available, else derived from eval/train loss. The **cost axis is user-pickable** (radio above the scatter):

    - `wall_clock_seconds` (default, the honest one) — measured `completed_at - started_at` for the cell. Cells that haven't finished yet (or completed without timestamps) sit out the scatter and surface as `cost pending` in the row list rather than being fabricated onto the plot.
    - `lora_r` — adapter footprint proxy. Cheap to compute and stable across in-progress sweeps, but a fiction when `base_model` is one of the swept axes (rank-16 on a 135M base and rank-16 on a 3B base look the same here when they aren't).
    - `base_params_m` — base-model parameter count in millions. The right axis when the sweep varies `base_model` and you're reading the Pareto as "which model size is worth the quality gain". Falls back to `cost pending` (`cost_source="unknown_base_model"`) for models not in the catalog.
