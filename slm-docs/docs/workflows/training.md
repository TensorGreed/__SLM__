...
> **Status:** the registry, recipe field, resolution, and UI/manifest surfacing are wired and tested. The warm-start **trainer** (`backend/scripts/train_warmstart_checkpoint.py`) is built: it resolves a license-audited corpus for the checkpoint's `task_shape` (Dolly-15k, CC BY-SA 3.0, sliced by category), enforces a permissive-license gate, full-fine-tunes the base, runs an identity-swap fairness spot-check, and flips the manifest to `status: "available"` with training + fairness provenance. Run it with `python scripts/train_warmstart_checkpoint.py --checkpoint qa-base-135m` (add `--smoke` for a tiny synthetic-corpus dry run). Bases that haven't been trained yet stay `status: "planned"` and cold-start cleanly. Scaling the corpus toward the full ~32 GPU-hour aspiration and publishing under `TensorGreed/` is follow-up work.
