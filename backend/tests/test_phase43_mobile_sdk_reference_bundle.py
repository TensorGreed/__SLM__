...
from app.services.deployment_target_service import (
    _validate_bundle_command_sanity,
...
class Phase43MobileSdkReferenceBundleTests(unittest.TestCase):
    ...
    def test_ios_reference_bundle_contains_expected_files_and_smoke_passes(self):
        ...
        required = {
            ...
            "scripts/smoke_inference.swift",
            "scripts/run_smoke.sh",
        }
        self.assertTrue(required.issubset(set(bundle_files)), bundle_files)
        self.assertIn("ios/ModelAssets/model.mlmodelc", list(artifact.get("model_placement_paths") or []))

        readme_path = Path(str(artifact.get("readme_path") or ""))
        self.assertTrue(readme_path.exists(), readme_path)
        readme_text = readme_path.read_text(encoding="utf-8")
        self.assertIn("Prerequisites", readme_text)
        self.assertIn("Model Placement", readme_text)
        self.assertIn("Run Instructions", readme_text)
        self.assertIn("Smoke Inference", readme_text)
        self.assertIn("bash scripts/run_smoke.sh", readme_text)
        ...
        smoke_checks = [dict(item) for item in list(smoke.get("checks") or []) if isinstance(item, dict)]
        command_checks = [row for row in smoke_checks if str(row.get("check_id") or "").startswith("command_sanity:")]
        self.assertTrue(command_checks, smoke_checks)
        self.assertTrue(all(bool(row.get("passed")) for row in command_checks), command_checks)
        ...
        self.assertEqual(set(members), set(bundle_files))
    ...
    def test_android_reference_bundle_contains_expected_files_and_smoke_passes(self):
        ...
        required = {
            ...
            "android/app/src/main/assets/models/.keep",
            "scripts/run_android_app.sh",
            "scripts/smoke_inference.kts",
            "scripts/run_smoke.sh",
        }
        self.assertTrue(required.issubset(set(bundle_files)), bundle_files)
        self.assertIn("android/app/src/main/assets/models/model.bin", list(artifact.get("model_placement_paths") or []))
    ...
    def test_command_sanity_detects_missing_script_paths(self):
        ok, details = _validate_bundle_command_sanity(
            bundle_dir=self.run_dir,
            command="bash scripts/missing_smoke.sh",
        )
        self.assertFalse(ok, details)
        missing = [str(item) for item in list(details.get("missing_paths") or [])]
        self.assertIn("scripts/missing_smoke.sh", missing, details)
